...
public class CreateUserAdaptorServiceImpl extends BaseAdaptor implements AdaptorService {
    ...
    public Map<String, Object> execute(Map<String, String> paraMap, Map<String, String> serviceMap) {
        ...
        try {
            ...
            String firstName = userInfoTO.getFirstName();
            String lastName = userInfoTO.getLastLame();
            String role = userInfoTO.getRole();
            String email = "";
            String requestStr = null;
            RequestObject reqObject = new RequestObject("", "", "", "", "", new Date(), "");
            reqObject.addRequestEntities(CommonIsvHelper.generateUserCreate(saasId, loginId, firstName, lastName, isUserRoleAdmin(role), email));
            ...
            returnCode = IsvConstants.ISV_ADAP_ERROR;
            log.error("[SaaSconnect] Error occured when create user by common adapter", e);
        }
        if (!IsvConstants.ISV_ADAP_SUCCESS.equalsIgnoreCase(returnCode)) {
            updateUserAssignStatus(isvService, IAppsConstants.PROVISION_REQUEST_PROVISION_FAILED);
        }
        responseMap.put(IAppsConstants.ISV_ADAPTOR_RETURN_CODE, returnCode);
        return responseMap;
    }

    public static boolean isUserRoleAdmin(String roleID) {
        if (null != roleID) {
            if (roleID.equalsIgnoreCase(IAppsConstants.ROLE_CUSTOMER_ADMIN)) {
                return true;
            }
        }
        return false;
