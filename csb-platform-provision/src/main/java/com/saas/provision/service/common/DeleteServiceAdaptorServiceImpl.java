...
public class DeleteServiceAdaptorServiceImpl extends DeleteAdaptor {
    ...
    public Map<String, Object> executeProductLogic(Map<String, String> paraMap, Map<String, String> serviceMap) {
        ...
        try {
            ...
            reqObject.addRequestEntities(CommonIsvHelper.generateProductDelete(saasId, "", 0, false));
                    ...
                    if (null != responseList && responseList.size() > 0) {
                        for (ResponseResult result : responseList) {
                            if (IsvConstants.ISV_RETURN_CODE_PS_0000.equalsIgnoreCase(result.getReturnCode())) {
                                ...
                                responseMap.put(IAppsConstants.ISV_ADAPTOR_RETURN_DETAIL_MSG, itemsMap.get(com.saas.webservices.config.IsvConstants.CUSTOMER_ITEM_SAASID));
            ...
            returnCode = IsvConstants.ISV_ADAP_ERROR;
            log.error("[SaaSconnect] Error occured when delete service by common adapter", e);
        }
    ...
    private String getRequestContent(String requestUrl, NameValuePair[] data ) throws UnsupportedEncodingException{
        ...
        for(int i=0; i<data.length;i++){
            if (data[i].getValue() != null && !"".equals(data[i].getValue())) {
				stringBuffer.append(data[i].getName()).append("=").append(java.net.URLEncoder.encode(data[i].getValue(),"utf-8"));
			} else {
