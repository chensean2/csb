...
public class CreateServiceAdaptorServiceImpl extends BaseAdaptor implements AdaptorService {
    ...
    public Map<String, Object> execute(Map<String, String> paraMap, Map<String, String> serviceMap) {
        ...
        try {
            CustomerInfoTO customerInfoTO = getCustomerInfo(isvService);
            String saasId = customerInfoTO.getSaasId();
            String domainName = customerInfoTO.getDomainName();
            String companyName = customerInfoTO.getCompanyName();
            ServiceTO serviceTO = new ServiceTO();
            serviceTO.setServiceId(customerInfoTO.getServiceId());
            ...
            reqObject.addRequestEntities(CommonIsvHelper.generateCustomerCreate(saasId, companyName, domainName));
            reqObject.addRequestEntities(CommonIsvHelper.generateProductCreate(saasId, "", 0, false));
                    ...
                    if (null != responseList && responseList.size() > 0) {
                        for (ResponseResult result : responseList) {
                            if (IsvConstants.ISV_RETURN_CODE_PS_0000.equalsIgnoreCase(result.getReturnCode())) {
                                ...
                                responseMap
                                        .put(IAppsConstants.ISV_ADAPTOR_RETURN_DETAIL_MSG, itemsMap
                                                .get(com.saas.webservices.config.IsvConstants.CUSTOMER_ITEM_SAASID));
            ...
            returnCode = IsvConstants.ISV_ADAP_ERROR;
            log.error("[SaaSconnect] Error occured when create service by common adapter", e);
        }
        if (!IsvConstants.ISV_ADAP_SUCCESS.equalsIgnoreCase(returnCode)) {
            updateProductOrderStatus(isvService, IAppsConstants.PACKAGE_ORDER_STATUS_FAILED);
        }
