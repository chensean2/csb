...
public class UpdateServiceAdaptorServiceImpl extends BaseAdaptor implements AdaptorService {
    ...
    public Map<String, Object> execute(Map<String, String> paraMap, Map<String,String> serviceMap) {
		 ...
		 RequestObject reqObject = new RequestObject(userId, password, application, env, adaptVersion,
                new Date(), requestNo);
		 reqObject.addRequestEntities(CommonIsvHelper.generateCustomerUpdate(saasId, custName, custDomain, trialind));
		 reqObject.addRequestEntities(CommonIsvHelper.generateProductUpdate(saasId, prodId, "newProdId", qty, trialind));
