...
public class EventProducer {
    ...
    public <T extends Entity> void addEntity(T entity) {
        if (enabled) {
            producer.asysaasendBody(queueName, createContext(entity, EventStatus.Add));
        }
    }
    ...
    public <T extends Entity> void updateEntity(T entity) {
        if (enabled) {
            producer.asysaasendBody(queueName, createContext(entity, EventStatus.Update));
        }
    ...
    public <T extends Entity> void removeEntity(T entity) {
        if (enabled) {
            producer.asysaasendBody(queueName, createContext(entity, EventStatus.Remove));
        }
    }
