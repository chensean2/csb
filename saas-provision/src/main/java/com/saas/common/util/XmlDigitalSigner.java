/*
 * =========================================================================
 *  Program Name: com.saas.common.util.XmlDigitalSigner
 *
...
/**
 * This helper class, part of the SAML-based Single Sign-On Reference Tool, serves to digitally sign XML files, given
 * the contents of the XML file, and a pair of public and private keys. The file is signed as per the specifications
 * defined by SAML 2.0.
 * 
 */
public class XmlDigitalSigner {
    ...
    /*
     * Signs the provided element with the specified private key and embeds the public key.
     */
    private static Element signSamlElement(Element element, PrivateKey privKey, PublicKey pubKey) throws SCException {
        ...
        try {
            ...
            XMLSignatureFactory sigFactory = XMLSignatureFactory.getInstance("DOM", (Provider) Class.forName(
                    providerName).newInstance());

            // Create a Reference to the enveloped document (we are
            // signing the whole document, so a URI of "" signifies that) and
            // also specify the SHA1 digest algorithm and the ENVELOPED Transform.
            List<?> envelopedTransform = Collections.singletonList(sigFactory.newTransform(Transform.ENVELOPED,
            ...
            if (pubKey instanceof DSAPublicKey) {
                ...
                throw new SCException("Error signing SAML element: Unsupported type of key");
            }

            CanonicalizationMethod canonicalizationMethod = sigFactory.newCanonicalizationMethod(
                    CanonicalizationMethod.INCLUSIVE_WITH_COMMENTS, (C14NMethodParameterSpec) null);

            // Create the SignedInfo
            SignedInfo signedInfo = sigFactory.newSignedInfo(canonicalizationMethod, signatureMethod, Collections
                    .singletonList(ref));
            ...
            // representation)
            org.w3c.dom.Element w3cElement = SsoUtil.toDom(element);
            ...
            return SsoUtil.toJdom(w3cElement);

        } catch (ClassNotFoundException e) {
            throw new SCException("Error signing SAML element: " + e.getMessage());
    ...
    /**
     * Signs the specified xmlString with the pair of provided keys, as per the SAML 2.0 specifications. Returns String
     * format of signed XML if successfully signed, returns null otherwise.
     * 
     * @param samlResponse
     *            SAML Response XML file to be signed
     * @param publicKey
     *            public key to read the signed XML
     * @param privateKey
     *            private key to sign the XML
     * @return String format of signed XML if signed correctly, null otherwise
     */
    public static String signXML(String samlResponse, PublicKey publicKey, PrivateKey privateKey) throws SCException {
        Document doc = SsoUtil.createJdomDoc(samlResponse);
        if (doc != null) {
            ...
            throw new SCException("Error signing SAML Response: Null document");
        }
    }
